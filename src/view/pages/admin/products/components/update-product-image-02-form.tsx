...
export type TUpdateProductImage02Form = z.infer<typeof updateProductImageSchema>

export function UpdateProductImage02Form({ product, onOpenChange }: TProps) {
  const queryClient = useQueryClient()
  const { updateProductImageFn } = useUpdateProductImage()
  const [previewImage, setPreviewImage] = useState<string | null>(
    product.productImage[1]?.url || null,
  )
  const form = useForm<TUpdateProductImage02Form>({
  ...
  function handleResetImage() {
    setPreviewImage(product.productImage[1]?.url || null)
      ...
      <form
        onSubmit={form.handleSubmit(() =>
          handleUpdateProductImage({
            data: form.getValues(),
            imageId: product.productImage[1]?.id || '',
            queryClient,
            onOpenChange,
